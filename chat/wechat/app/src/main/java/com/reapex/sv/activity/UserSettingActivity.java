 ...
 * @author zhou
 */
public class UserSettingActivity extends L0_BaseActivity {
...
//sv         mUserDao = new UserDao();
    ...
    private void initView() {
        if (L0_Constant.IS_NOT_FRIEND.equals(mIsFriend)) {
            // 非好友
    ...
    private void loadData(User user) {
        // 备注
        mEditContactTv.setText(user.getUserName());
        // 星标好友
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.rl_add_to_home_screen:
                ...
                intent = new Intent(UserSettingActivity.this, L6_ChatActivity.class);
                intent.setAction("com.android.launcher.action.INSTALL_SHORTCUT");
                intent.putExtra("targetType", L0_Constant.TARGET_TYPE_SINGLE);
                intent.putExtra("contactId", mContact.getUserId());
                intent.putExtra("contactNickName", mContact.getUserName());
                intent.putExtra("contactAvatar", mContact.getUserAvatar());
                new Thread(() -> {
                    Bitmap bitmap = getBitmapFromAvatarUrl(mContact.getUserAvatar());
                    addShortcut(UserSettingActivity.this, mContact.getUserName(), bitmap, intent);
                }).start();
                break;
            case R.id.iv_add_star:
                setContactStarred(L0_Constant.CONTACT_IS_STARRED);
                break;
            case R.id.iv_cancel_star:
                setContactStarred(L0_Constant.CONTACT_IS_NOT_STARRED);
                break;
            case R.id.iv_block:
                setContactBlocked(L0_Constant.CONTACT_IS_BLOCKED);
                break;
            case R.id.iv_cancel_block:
                setContactBlocked(L0_Constant.CONTACT_IS_NOT_BLOCKED);
                break;
            case R.id.rl_delete:
                // 删除联系人
                confirmDialog = new ConfirmDialog(UserSettingActivity.this, "删除联系人",
                        "将联系人\"" + mContact.getUserName() + "\"删除，将同时删除与该联系人的聊天记录",
    ...
    private void setContactStarred(final String isStarred) {
        ...
        String url = L0_Constant.BASE_URL + "users/" + mUser.getUserId() + "/contacts/" + mContactId + "/star";
        Map<String, String> paramMap = new HashMap<>();
        paramMap.put("isStarred", isStarred);

        mVolleyUtil.httpPutRequest(url, paramMap, response -> {
            mDialog.dismiss();
            if (L0_Constant.CONTACT_IS_STARRED.equals(isStarred)) {

                mAddStarIv.setVisibility(View.GONE);
                mCancelStarIv.setVisibility(View.VISIBLE);

                //sv mContact.setIsStarred(L0_Constant.CONTACT_IS_STARRED);
                ...
                //sv mContact.setIsStarred(L0_Constant.CONTACT_IS_NOT_STARRED);
                Toast.makeText(UserSettingActivity.this, "已取消星标朋友", Toast.LENGTH_SHORT).show();
            }
            //sv mUserDao.saveUser(mContact);
    ...
    private void setContactBlocked(final String isBlocked) {
        ...
        String url = L0_Constant.BASE_URL + "users/" + mUser.getUserId() + "/contacts/" + mContactId + "/block";
        Map<String, String> paramMap = new HashMap<>();
        paramMap.put("isBlocked", isBlocked);

        mVolleyUtil.httpPutRequest(url, paramMap, response -> {
            mDialog.dismiss();
            if (L0_Constant.CONTACT_IS_BLOCKED.equals(isBlocked)) {

                mBlockIv.setVisibility(View.GONE);
                mCancelBlockIv.setVisibility(View.VISIBLE);

                //sv mContact.setIsBlocked(L0_Constant.CONTACT_IS_BLOCKED);
                ...
                //sv mContact.setIsBlocked(L0_Constant.CONTACT_IS_NOT_BLOCKED);
            }
            //sv mUserDao.saveUser(mContact);
        }, volleyError -> mDialog.dismiss());
    ...
    private void deleteContact(final String userId, final String contactId) {
        ...
        mVolleyUtil.httpDeleteRequest(url, response -> {
            ...
            // 清除本地记录
            // 通讯录删除
//            L_UserBean user = mUserDao.getUserById(contactId);
  //          L_UserBean.delete(user);
