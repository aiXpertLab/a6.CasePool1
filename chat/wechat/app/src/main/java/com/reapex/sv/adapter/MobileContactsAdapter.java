...
import com.reapex.sv.activity.NewFriendsApplyConfirmActivity;
import com.reapex.sv.activity.UserInfoStrangerActivity;
...
//sv         viewHolder.mWechatNameTv.setText("微信:" + user.getUserName());

        //sv    String header = user.getUserHeader();
...
*/
        final boolean isFriend = true; //sv mUserDao.checkIsFriend(user.getUserPhone());
        if (isFriend) {
            // 是好友
        ...
        viewHolder.mRootLl.setOnClickListener(view -> {
            if (isFriend) {
                ...
                intent.putExtra("from", L0_Constant.CONTACTS_FROM_CONTACT);
                mContext.startActivity(intent);
            }
        });
