...
import com.reapex.sv.utils.DistanceUtil;
...
public class PeopleNearbyAdapter extends ArrayAdapter<PeopleNearby> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        viewHolder.mNameTv.setText(peopleNearby.getUserName());
        ...
        if (mSexFilter) {
            ...
            if (L0_Constant.USER_SEX_MALE.equals(peopleNearby.getUserName())) {
                viewHolder.mSexIv.setImageResource(R.mipmap.icon_sex_male);
            } else if (L0_Constant.USER_SEX_FEMALE.equals(peopleNearby.getUserName())) {
        ...
        if (TextUtils.isEmpty(peopleNearby.getSerial())) {
            viewHolder.mWhatsupTv.setVisibility(View.GONE);
        } else {
            viewHolder.mWhatsupTv.setText(peopleNearby.getSerial());
        }
        PositionInfo myPositionInfo = L0_SharedPreferences.getInstance().getPositionInfo();
