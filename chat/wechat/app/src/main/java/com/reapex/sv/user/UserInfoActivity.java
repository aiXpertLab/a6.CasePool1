...
import com.reapex.sv.activity.ContactPrivacyActivity;
import com.reapex.sv.activity.EditContactActivity;
import com.reapex.sv.L6_ChatActivity;
import com.reapex.sv.activity.UserInfoMoreActivity;
import com.reapex.sv.activity.UserSettingActivity;
 ...
 * @author zhou
 */
public class UserInfoActivity extends L0_BaseActivity {
...
//sv        mVolleyUtil = VolleyUtil.getInstance(this);
...
//sv         mContact = mUserDao.getUserById(mContactId);
        loadData(mContact);
//sv        getContactFromServer(mUser.getUserId(), mContactId);
    }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.iv_setting:
                ...
                intent.putExtra("isFriend", L0_Constant.IS_FRIEND);
            ...
            case R.id.rl_desc:
                ...
                intent.putExtra("contactId", mContact.getUserId());
                //sv intent.putExtra("isFriend", mContact.getIsFriend());
                startActivity(intent);
                break;
            // 朋友圈
...
//                startActivity(intent);
            ...
            case R.id.rl_operate:
                intent = new Intent(UserInfoActivity.this, L6_ChatActivity.class);
    ...
    private void loadData(User user) {
        if (!TextUtils.isEmpty(user.getUserAvatar())) {
            // 渲染朋友圈图片
            List<String> circlePhotoList = L_CommonUtil.getListFromJson(user.getUserAvatar(), String.class);
        ...
        if (!TextUtils.isEmpty(user.getUserId())) {
            mWxIdTv.setText("微信号：" + user.getUserId());
        }

        List<String> mobileList;
        try {
            mobileList = JSON.parseArray(user.getUserAvatar(), String.class);
            if (null == mobileList) {
        ...
        if (L0_Constant.PRIVACY_CHATS_MOMENTS_WERUN_ETC.equals(user.getUserAvatar())) {
            // 所有权限
            if (L0_Constant.HIDE_MY_POSTS.equals(user.getUserAvatar())) {
                privacyList.add("不让他看朋友圈");
            }

            if (L0_Constant.HIDE_HIS_POSTS.equals(user.getUserName())) {
                privacyList.add("不看他的朋友圈");
            }

            if (!L0_Constant.HIDE_HIS_POSTS.equals(user.getUserName()) &&
                    !L0_Constant.HIDE_MY_POSTS.equals(user.getUserAvatar())) {
                privacyList.add("");
            }
